...
// Verify the create New is visible
  ...
  const projectTitle = credentials.simpleProjectTitle;
...
//Visibility
  await page.locator('iframe[name="RTM20_IN1"]').contentFrame().locator('label').filter({ hasText: 'Immediate' }).click();
...
//Add Distribution
  ...
  const confirmButton = await page.locator('iframe[name="RTM20_IN1"]').contentFrame().getByRole('button', { name: 'Confirm' });
...
//Filter
  ...
  await page.locator('iframe[name="RTM20_IN1"]').contentFrame().locator('#projectTitleKey').click();
  const projectTitleEdit = credentials.SimpleProjectTitleEdit;
